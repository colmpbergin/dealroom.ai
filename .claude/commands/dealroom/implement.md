...
Execute an implementation plan created by `/create-plan`. Read the plan
thoroughly, execute each step in order, and report on the completed work.

## Variables

plan_path: $ARGUMENTS (path to the plan file, e.g.,
...
1. **Follow the Step-by-Step Tasks in exact order.**
   ...
   - If a step involves modifying a file, read the file first, then apply
     changes precisely
...
3. **Handle issues gracefully:**
   - If a step can't be completed as written, note the issue and adapt if the
     intent is clear
